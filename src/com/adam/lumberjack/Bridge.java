...
public class Bridge extends GameObject {
    ...
    public Bridge(World world, float x, float y, TextureRegion texture, TextureRegion destroyed, float widthRatio, float heightRatio, Hitbox hitbox, int maxHealth, int loot, Chunk ownerChunk) {
    ...
    protected void createBody(World world, float x, float y) {
        ...
        fixtureDef.filter.categoryBits = ObjectCategory.BRIDGE.getValue();
        fixtureDef.filter.maskBits = (short) (ObjectCategory.PLAYER_SENSOR.getValue() | ObjectCategory.PLAYER.getValue() | ObjectCategory.MOB.getValue() | ObjectCategory.MOB_SENSOR.getValue());
